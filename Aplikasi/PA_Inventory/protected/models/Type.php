...
class Type extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('code_type', 'length', 'max'=>100),
			array('name', 'length', 'max'=>200),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, code_type, name', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'importInventories' => array(self::HAS_MANY, 'ImportInventory', 'id_type'),
			'inventories' => array(self::HAS_MANY, 'Inventory', 'id_type'),
		);
	}
